...
const form = useForm({
    ...
    currency:              'EUR',
    chantier_address:      '',
    payment_conditions:    '30% à la commande, solde à la livraison',
    notes:                 '',
    footer:                '',
    items: [{ product_id: null, description: '', unit_price: 0, unit: 'm²', quantity: 1, vat_rate: 0 }],
})
...
const onProductChange = (index, products) => {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-xl font-semibold text-gray-800">Nouveau devis</h2>
            </div>
        </template>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow p-6 grid grid-cols-2 gap-4">
                    ...
                    <div class="col-span-2">
                        ...
                        <InputError class="mt-1" :message="form.errors.estimated_start_date" />
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow p-6">
                    ...
                    <InputError class="mt-1" :message="form.errors.chantier_address" />
                </div>
                ...
                <div class="bg-white rounded-xl shadow p-6">
                    ...
                    <div v-for="(item, index) in items" :key="index" class="grid grid-cols-12 gap-2 mb-3 items-start">
                        <div class="col-span-3">
                            <select v-model="item.product_id" @change="onProductChange(index, products)"
                        ...
                        <div class="col-span-3">
                            <TextInput v-model="item.description" type="text" placeholder="Désignation *" class="block w-full text-sm" required />
                        </div>
                        <div class="col-span-2">
                            <TextInput v-model="item.unit_price" type="number" step="0.01" min="0" placeholder="0.00" class="block w-full text-sm" required />
                        </div>
                        <div class="col-span-1">
                            <TextInput v-model="item.quantity" type="number" step="0.001" min="0.001" placeholder="1" class="block w-full text-sm" required />
                        </div>
                        <div class="col-span-1">
                            <TextInput v-model="item.unit" type="text" placeholder="m²" class="block w-full text-sm" />
                        </div>
                ...
                <div class="bg-white rounded-xl shadow p-6 grid grid-cols-2 gap-4">
                    <div class="col-span-2">
                        ...
                        <textarea id="payment_conditions" v-model="form.payment_conditions" rows="2"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" />
                        <InputError class="mt-1" :message="form.errors.payment_conditions" />
                    </div>
                    <div>
                ...
                <div class="flex justify-end">
                    <PrimaryButton :disabled="form.processing">
                        {{ form.processing ? '...' : 'Créer le devis' }}
                    </PrimaryButton>
                </div>
            </form>
        </div>
