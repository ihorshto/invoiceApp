    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-xl font-semibold text-gray-800">Nouvelle facture / Новий рахунок</h2>
            </div>
        </template>
        ...
        <div class="py-8 max-w-5xl mx-auto px-4">
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow p-6 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="client_id" v-model="form.client_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">— Sélectionner / Обрати —</option>
                ...
                <div class="bg-white rounded-xl shadow p-6">
                    ...
                    <div v-for="(item, index) in items" :key="index" class="grid grid-cols-12 gap-2 mb-3 items-start">
                        <div class="col-span-3">
                            <select v-model="item.product_id" @change="onProductChange(index)"
                                ...
                                <option :value="null">— Produit / Товар —</option>
                        ...
                        <div class="col-span-2">
                            <TextInput v-model="item.unit_price" type="number" step="0.01" min="0" placeholder="Prix HT" class="block w-full text-sm" required />
                        </div>
                        <div class="col-span-1">
                            <TextInput v-model="item.quantity" type="number" step="0.001" min="0.001" placeholder="Qté" class="block w-full text-sm" required />
                        </div>
                        <div class="col-span-1">
                            <TextInput v-model="item.vat_rate" type="number" step="0.01" min="0" max="100" placeholder="TVA%" class="block w-full text-sm" required />
                        </div>
                    ...
                    <button type="button" @click="addItem" class="text-blue-600 text-sm hover:underline mt-2">+ Ajouter ligne / Додати рядок</button>
                ...
                <div class="bg-white rounded-xl shadow p-6 grid grid-cols-2 gap-4">
                    <div>
                        <InputLabel for="notes" value="Notes / Нотатки" />
                        <textarea id="notes" v-model="form.notes" rows="3"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" />
                    </div>
                    <div>
                        <InputLabel for="footer" value="Pied de page / Нижній колонтитул" />
                ...
                <div class="flex justify-end">
                    <PrimaryButton :disabled="form.processing">
                        {{ form.processing ? '...' : 'Créer / Створити' }}
                    </PrimaryButton>
                </div>
            </form>
        </div>
